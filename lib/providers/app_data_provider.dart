...
class AppData extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Future<String?> checkIn(Venue venue) async {
    ...
    DateTime currentSyncTime = lastSync;

    // Simulate _recalculateEnergy logic
    if (currentPoints < 10) {
      final intervals = diff.inMinutes ~/ 30;
      if (intervals > 0) {
        currentPoints = (currentPoints + intervals).clamp(0, 10);
        currentSyncTime = lastSync.add(Duration(minutes: intervals * 30));
      }
    } else {
      currentSyncTime = now;
    }

    // 2. Check if enough points AFTER recalculation
    if (currentPoints < 1) {
